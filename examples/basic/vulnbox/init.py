...
""")
    ...
    response = subprocess.run(["service", "app", "status"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    if response.returncode != 0:
        print("error", response.stderr)
        return
    
    # Request the homepage of the service
    response = subprocess.run(["curl", f"http://{teamIp}:5000/"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    if response.returncode != 0:
        print("error", response.stderr)
        return
